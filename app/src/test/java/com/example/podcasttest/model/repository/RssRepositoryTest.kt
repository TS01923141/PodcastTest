...
class RssRepositoryTest() {
    ...
    private val id = ""
    private val before = null

    @Before
    fun setUp(){
        MockKAnnotations.init(this)
        repository = RssRepository(service)
    }

    @Test
    fun `should return null when throws UnknownHostException`() {
        coEvery { service.getRss(id, before) } throws UnknownHostException()
        runBlocking {
            repository.refreshChannel(id, before)
            assertEquals(repository.channel.value, Channel())
        }
    }

    @Test
    fun `should return null when response not sucessful`() {
        coEvery { service.getRss(id, before).isSuccessful } returns false
        runBlocking {
            repository.refreshChannel(id, before)
            assertEquals(repository.channel.value, Channel())
        }
